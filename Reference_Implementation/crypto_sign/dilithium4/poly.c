...
void poly_freeze(poly *a) {
  ...
  poly_freeze_label87:for(i = 0; i < N; ++i)
...
void poly_add(poly *c, const poly *a, const poly *b)  {
  ...
  poly_add_label74:for(i = 0; i < N; ++i)
...
void poly_sub(poly *c, const poly *a, const poly *b) {
  ...
  poly_sub_label75:for(i = 0; i < N; ++i)
...
void poly_shiftl(poly *a) {
  ...
  poly_shiftl_label76:for(i = 0; i < N; ++i)
    a->coeffs[i] <<= D;
                  ...
                  uint16_t nonce)
{
  ...
  while(ctr < N) {
    off = buflen % 3;
    poly_uniform_label77:for(i = 0; i < off; ++i)
...
#error "rej_eta() assumes ETA <= 7"
  ...
  ctr = pos = 0;
  rej_eta_label78:while(ctr < len && pos < buflen) {
#if ETA <= 3
    t0 = buf[pos] & 0x07;
    t1 = buf[pos++] >> 5;
#else
...
#error "rej_gamma1m1() assumes GAMMA1 - 1 fits in 19 bits"
  ...
  ctr = pos = 0;
  rej_gamma1m1_label79:while(ctr < len && pos + 5 <= buflen) {
    t0  = buf[pos];
...
#if 2*ETA <= 7
  polyeta_pack_label80:for(i = 0; i < N/8; ++i) {
...
#if 2*ETA <= 7
  polyeta_unpack_label81:for(i = 0; i < N/8; ++i) {
...
#error "polyt1_pack() assumes D == 14"
  ...
  polyt1_pack_label82:for(i = 0; i < N/8; ++i) {
...
void polyt0_pack(unsigned char *r, const poly *a) {
  ...
  polyt0_pack_label83:for(i = 0; i < N/4; ++i) {
...
void polyt0_unpack(poly *r, const unsigned char *a) {
  ...
  polyt0_unpack_label84:for(i = 0; i < N/4; ++i) {
...
#error "polyz_pack() assumes GAMMA1 <= 2^{19}"
  ...
  polyz_pack_label85:for(i = 0; i < N/2; ++i) {
    /* Map to {0,...,2*GAMMA1 - 2} */
...
void polyw1_pack(unsigned char *r, const poly *a) {
  ...
  polyw1_pack_label86:for(i = 0; i < N/2; ++i)
